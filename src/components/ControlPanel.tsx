...
import { Bookmark, Eye, Heart, Image, RefreshCw, Settings, Type } from "lucide-react";
...
function ControlButton({
  ...
  const { icon: Icon, label, loading, special, shortcut } = config;
  const showFilledHeart = special && isSaved;
  ...
  return (
    <div className="relative group">
      ...
      >
        {special && <ParticleBurst active={burstKey > 0} triggerKey={burstKey} />}
        <Icon
        ...
        <span
          ...
          style={{
            ...
            minWidth: special ? "36px" : undefined,
          }}
        >
          {label}
        </span>
      ...
      >
        {buttons.map((config) => (
          <ControlButton
            ...
            isHovered={hoveredButton === config.key}
            isSaved={!!isSaved}
            burstKey={burstKey}
            onMouseEnter={() => setHoveredButton(config.key)}
            onMouseLeave={() => setHoveredButton(null)}
            onClick={() => handleClick(config.key, config.onClick)}
          />
        ))}
      </div>
    </div>
